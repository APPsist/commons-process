package de.appsist.commons.process.bpmn;
...
/**
 * Implementation for APPsist service call annotations in a BPMN model.
...
public class BPMNServiceCallAnnotation implements ServiceCallAnnotation {
	 ...
	 * @param serviceCallElement XML element for a <code>seviceCall</cde> as specified in appsist:bpmn:annotations.
	 */
	public BPMNServiceCallAnnotation(Element serviceCallElement) {
		this.serviceCallElement = serviceCallElement;
		Element triggerElement = serviceCallElement.getChild("trigger", BPMNNamespace.APPSIST);
	 ...
	 */
	public BPMNServiceCallAnnotation(String serviceId, String methodId, Type type, HttpMethod httpMethod) {
		serviceCallElement = new Element("serviceCall", BPMNNamespace.APPSIST);
	 ...
	 * @return XML data structure as specified for <code>event</code> in appsist:bpmn:annotations.
	 */
	...
	public void setInputMapping(Map<String, String> mapping) {
		Element inputElement = serviceCallElement.getChild("input", BPMNNamespace.APPSIST);
		if (inputElement != null) {
			inputElement.removeChildren("entry", BPMNNamespace.APPSIST);
		} else {
			inputElement = new Element("input", BPMNNamespace.APPSIST);
			Element triggerElement = serviceCallElement.getChild("trigger", BPMNNamespace.APPSIST);
		...
		for (String key : mapping.keySet()) {
			...
			Element entryElement = new Element("entry", BPMNNamespace.APPSIST);
	...
	public void setOutputMapping(Map<String, String> mapping) {
		Element outputElement = serviceCallElement.getChild("output", BPMNNamespace.APPSIST);
		if (outputElement != null) {
			outputElement.removeChildren("entry", BPMNNamespace.APPSIST);
		} else {
			outputElement = new Element("output", BPMNNamespace.APPSIST);
	...
	public Map<String, String> getInputMapping() {
		...
		Element inputElement = serviceCallElement.getChild("input", BPMNNamespace.APPSIST);
		if (inputElement != null) {
			for (Element entryElement : inputElement.getChildren("entry", BPMNNamespace.APPSIST)) {
	...
	public Map<String, String> getOutputMapping() {
		...
		Element outputElement = serviceCallElement.getChild("output", BPMNNamespace.APPSIST);
		if (outputElement != null) {
			for (Element entryElement : outputElement.getChildren("entry", BPMNNamespace.APPSIST)) {
				String key = entryElement.getAttributeValue("key");
				String reference = entryElement.getAttributeValue("reference");
	...
	private void setHttpMethod(HttpMethod httpMethod) {
		serviceCallElement.setAttribute("httpMethod", httpMethod.toString());
	}
	
	@Override
	public HttpMethod getHttpMethod() {
		String method = serviceCallElement.getAttributeValue("httpMethod");
