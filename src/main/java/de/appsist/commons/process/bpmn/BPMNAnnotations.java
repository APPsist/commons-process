...
/**
 * Wrapper for BPMN annotations specified in appsist:bpmn:annotations.
...
public class BPMNAnnotations implements Annotations {
	...
	/**
	 * Parses a BPMN <code>extensionElements</code> element to retrieve APPsist annotations.  
	...
	public BPMNAnnotations(Element extensionElementsElement) {
		...
		for (Element extensionElement : extensionElementsElement.getChildren()) {
			if (extensionElement.getNamespace() != BPMNNamespace.APPSIST) {
				continue;
			}
			...
			case "events":
				...
				for (Element eventElement : eventsElement.getChildren("event", BPMNNamespace.APPSIST)) {
					eventAnnotations.add(new BPMNEventAnnotation(eventElement));
				}
				break;
			case "serviceCalls":
				serviceCallsElement = extensionElement;
				for (Element serviceCallElement : serviceCallsElement.getChildren("serviceCall", BPMNNamespace.APPSIST)) {
					serviceCallAnnotations.add(new BPMNServiceCallAnnotation(serviceCallElement));
				}
				break;
			case "triggers":
				triggersElement = extensionElement;
				for (Element triggerElement : triggersElement.getChildren("trigger", BPMNNamespace.APPSIST)) {
		...
		if (eventsElement == null) {
			eventsElement = new Element("events", BPMNNamespace.APPSIST);
		}
		if (serviceCallsElement == null) {
			serviceCallsElement = new Element("serviceCalls", BPMNNamespace.APPSIST);
		}
		if (triggersElement == null) {
			triggersElement = new Element("triggers", BPMNNamespace.APPSIST);
		}
